  ...
  http.get('http://127.0.0.1:9222/json', (res) => {
    ...
    res.on('end', async () => {
      ...
      ws.addEventListener('open', async () => {
        ...
        await setViewport(1280, 1050);

        // Pride & Prejudice Page 1
        await evaluate(`
          new Promise((resolve) => {
            const check = () => {
              if (window.nookApp && window.nookApp.catalog && window.nookApp.catalog.length >= 105) {
                ...
                window.nookApp.navigateTo('reader', { bookId: 'pride-and-prejudice', chapterNumber: 1 });
                const checkReader = () => {
        ...
        await screenshot('full_pride_page1_1050h.png');

        // Next page (Page 2)
        await evaluate(`
          (() => {
            const btn = document.querySelector('#readerPaperCornerNext');
            if (btn) btn.click();
          })()
        `);
        await new Promise((r) => setTimeout(r, 800));
        await screenshot('full_pride_page2_1050h.png');
        ...
        `);
        await screenshot('full_frankenstein_letter1_1050h.png');

        // Frankenstein Letter I Page 2
        await evaluate(`
          (() => {
            const btn = document.querySelector('#readerPaperCornerNext');
            if (btn) btn.click();
        ...
        `);
        await new Promise((r) => setTimeout(r, 800));
        await screenshot('full_frankenstein_letter2_1050h.png');
